...
puzinput = """toggle 461,550 through 564,900
...
def serData(data):
    ...
    for line in data.split("\n"):
        ...
        if splitLine[0] == "toggle":
            ...
            instruction.append([int(val) for val in splitLine[1].split(",")])
            instruction.append([int(val) for val in splitLine[3].split(",")])
        else:
            if splitLine[1] == "on": instruction.append("on")
            else: instruction.append("off")

            instruction.append([int(val) for val in splitLine[2].split(",")])
            instruction.append([int(val) for val in splitLine[4].split(",")])
        output.append(instruction) 
    
    return output

def calculateOn(data):
    grid = [[False]*1000 for i in range(1000)]

    for instruction in data:
        lamfunc = None
        if instruction[0] == "toggle":
            lamfunc = lambda x: not x
        elif instruction[0] == "on":
            lamfunc = lambda x: True
        else:
            lamfunc = lambda x: False
        
        x1, y1 = instruction[1]
        x2, y2=  instruction[2]
        for i in range(x1, x2+1):
            for j in range(y1, y2+1):
                grid[i][j] = lamfunc(grid[i][j])
...
print(calculateOn(serData(puzinput)))