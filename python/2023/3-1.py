...
input_data = """.......855.442......190..................................969..........520.......59.............................................172..........
...
def serialize_string(string):
    return [list(elt) for elt in string.split("\n")]

def adj_symbol(surrounding_lines, i):
    str_digits = [str(n) for n in range(10)]
    surrounding_chars = [surrounding_lines[j][i-1] for j in range(3)]
    number = ""
    # print(sur_lines)
    # input()
    
    is_number = surrounding_lines[1][i] in str_digits
    while is_number:
        surrounding_chars += [surrounding_lines[0][i], surrounding_lines[2][i]]
        number += surrounding_lines[1][i]

        i += 1
        is_number = surrounding_lines[1][i] in str_digits

    surrounding_chars += [surrounding_lines[j][i] for j in range(3)]        

    has_symbol = any([char not in str_digits + ["."] for char in surrounding_chars])

    return has_symbol, number


def sum_schematic_nums(input_data):

    # we do this to avoid index errors when we get to an edge of the table of values    
    margined_input_data = [["."]+line+["."] for line in input_data]
    margined_input_data = [["."]*len(margined_input_data[0])] + margined_input_data + [["."]*len(margined_input_data[0])]

    str_digits = [str(i) for i in range(10)]

    final_sum = 0
    i = 1
    while i < len(margined_input_data)-1:

        j = 1
        while j < len(margined_input_data[i])-1:
            # print(margined_input_data[i][j] in str_digits)
            if margined_input_data[i][j] in str_digits:
                is_adj, number = adj_symbol([margined_input_data[k] for k in range(i-1, i+2)], j)

                if is_adj:
                    final_sum += int(number)

                # skip the rest of the indices blonging to the same number
                j += len(number)
...
# open("textinput.txt", "w").write("\n".join([str(lst) for lst in serialized_data]))
# print(serialize_string(example_input_data))
print(sum_schematic_nums(serialize_string(example_input_data))) 
print(sum_schematic_nums(serialize_string(input_data)))