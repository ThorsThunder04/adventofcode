calibrations = """sixrrmlkptmc18zhvninek
jcb82eightwond
...
def get_calibration(calibrations: str) -> int:
    ...
    for cal in calibrations:
        ...
        while i < len(cal):
            for key in string_digits.keys():
                if (i + len(key)-1) < len(cal):
                    if cal[i:i+len(key)] == key:
                        digits.append(string_digits[key])
            
            if cal[i] in string_digits.values():
                digits.append(cal[i])
            
            i += 1

        calibrations_sum += int(digits[0])*10 + int(digits[-1])
    
    return calibrations_sum


print(get_calibration(calibrations.split("\n")))