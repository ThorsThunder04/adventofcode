...
def adj_gear(sur_lines, ast_i):
    str_digits = [str(n) for n in range(10)]
    numbers = []
    ratio = 0
    
    for i in range(len(sur_lines)):
        is_adj = False
        number = ""
        for j in range(len(sur_lines[0])):
            
            if sur_lines[i][j] == "." and not is_adj:
                number = ""
            elif number and sur_lines[i][j] in (".", "*") and is_adj:
                numbers.append(number)
                number = ""
                is_adj = False
            elif sur_lines[i][j] in str_digits:
                number += sur_lines[i][j]
                if abs(ast_i-j) in (0, 1):
                    is_adj = True
...
def sum_schematic_nums(input_data):
    ...
    while i < len(margined_input_data)-1:
        ...
        while j < len(margined_input_data[i])-1:
            ...
            if margined_input_data[i][j] == "*":
                gear_ratio = adj_gear([margined_input_data[k] for k in range(i-1, i+2)], j)
