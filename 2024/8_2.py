        ...
        for c in range(gColMax):
            ...
            if isNode(grid[r][c]): # if on a node
                ...
                for rr,cc in otherNodes:
                    ...
                    # while anti-nodes of this frequency on downward axis are in bounds, log them
                    k = 0
                    while 0 <= rr + rd*k < gRowMax and 0 <= cc + cd*offset*k < gColMax:
                        nAntiNodes.add((rr+rd*k, cc + cd*offset*k))
                        k += 1
                    # same thing but for upward axis
                    k = 0
                    while 0 <= r - rd*k < gRowMax and 0 <= c - cd*offset*k < gColMax:
                        nAntiNodes.add((r-rd*k, c-cd*offset*k))
                        k += 1
