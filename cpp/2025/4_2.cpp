...
int main() {
    ...
    vector<array<int,2>> to_remove; // for some reason my compiler doesn't detect the pair class, so I'm using arrays
    int i = 0;
        ...
        for (int r = 0; r < (int)roll_grid.size(); r++) {
            for (int c = 0; c < (int)roll_grid[r].size(); c++) {
                ...
                if (roll_grid[r][c] == '@' && hm_adj(roll_grid, r, c) < 4) {
                    num_accessible++;
                    to_remove.push_back((array<int,2>){r,c});
                }
            }
        }

        while (i < (int)to_remove.size()) {
            roll_grid[to_remove[i][0]][to_remove[i][1]] = '.';
            i++;
        }
