game_input = """Game 1: 4 blue, 4 red, 16 green; 14 green, 5 red; 1 blue, 3 red, 5 green
...
def num_possible_games(input_games, colors):

    sum_of_possible = 0

    for game in input_games:
        num_each_color = {color:[] for color in colors.keys()}
        game_id = int(game[5:game.find(":")])
        pulls = game.split(":")[1][1:].split(";")

        for pull in pulls:
            for hand in pull.split(","):
                num, color = hand.strip().split(" ")
                num_each_color[color].append(int(num))

        # for key,value in num_each_color.items():
        #     if max(value) > colors[key]:
        #         game_id = 0 
        
        # sum_of_possible += game_id 

        power = 1
        for values in num_each_color.values():
            power *= max(values)
            
        sum_of_possible += power
...
print(num_possible_games(example_input.split("\n"), colors))

print(num_possible_games(game_input.split("\n"), colors))
